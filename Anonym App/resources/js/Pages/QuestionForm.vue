<template>
    ...
    <div class="h-screen w-full bg-white relative flex overflow-hidden">
        ...
        <!-- Header -->
        <header class="h-20 w-full flex items-center relative px-5 space-x-10 bg-blue-950">
        <!-- Informação -->
        ...
        <div class="flex flex-col items-end ">
            ...
            <!-- <Link class="bg-red-600 rounded-md text-white ml-2 w-full h-full p-2 font-bold" :href="route('logout')">Logout</Link> -->
        ...
        <!-- Container -->
        <div class="w-full m-4 rounded-tl grid-flow-col auto-cols-max gap-4 overflow-y-scroll">
        <!-- Container -->
        <div class="w-full flex flex-wrap gap-4">
            <div class="flex justify-center w-full">
                <h1 class="text-blue-950 text-2xl font-black">My Questions</h1>
                <!-- {{ $page.props.auth.user.name }} -->
            </div>

            <div class="w-96 h-60 rounded-2xl flex-shrink-0 flex-grow bg-sky-600 hover:bg-sky-500 transition-colors p-8 cursor-pointer"  v-for="(question, index) in data.myquestions" :key="question.id">
               <div class=" h-3/6" @click="show(question.id)">
                    <h3 class="text-lg font-bold text-center text-white">{{ question.title }}</h3>
                    <div class="flex mt-8 justify-center">

                        <svg viewBox="0 0 1024 1024" class="icon" version="1.1" height="20px" width="20px" xmlns="http://www.w3.org/2000/svg" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M789.333333 832H234.666667l-128 128V234.666667c0-70.4 57.6-128 128-128h554.666666c70.4 0 128 57.6 128 128v469.333333c0 70.4-57.6 128-128 128z" fill="#ffffff"></path></g></svg>

                        <h3 class=" pl-3 text-sm font-bold text-center text-white align-bottom flex justify-center">{{ question.commentcount }}</h3>
                    </div>
               </div>
               <div class="flex justify-center" v-if="editMode==true && Editid == question.id">
                <input type="text" v-model="question.title" class="w-fullphp ar" @keyup.enter="UpdateQuestion(question)"> 
               </div>
                <div class="w-full flex justify-center mt-2">
                    <h3 class="bg-blue-400 hover:bg-blue-500 mb-4 flex text-center justify-center items-center p-2 rounded-md ml-2 text-white cursor-pointer w-1/4" @click="EditQuestion(question)" v-if="Editid !== question.id">Edit</h3>
                    <h3 class="bg-red-400 hover:bg-red-500 mb-4 flex text-center justify-center items-center p-2 rounded-md ml-2 text-white cursor-pointer w-1/4" @click="DeleteQuestion(question.id, index)" v-if="Editid !== question.id">Delete</h3>
                    
                    <h3 class="bg-blue-400 hover:bg-blue-500 mb-4 flex text-center justify-center items-center p-2 rounded-md ml-2 text-white cursor-pointer w-1/4" @click="UpdateQuestion(question)"  v-if="Editid == question.id">Update</h3>
                    <h3 class="bg-red-400 hover:bg-red-500 mb-4 flex text-center justify-center items-center p-2 rounded-md ml-2 text-white cursor-pointer w-1/4" v-if="Editid == question.id" @click="CancelEdit" >Cancel</h3>
                </div>
        ...
        <div class=" w-full p-6 mx-auto my-auto align-bottom">
            <div class="pt-2 pb-2 flex align-bottom">
                <textarea type="text" class="border-sky-600 border-solid border-2 rounded-xl w-full h-16 p-2" v-model="inputQuestion" @keyup.ctrl.enter="newQuestion" required></textarea>
                <button class="bg-sky-600 rounded-md text-white ml-2 w-1/6 h-16" @click="newQuestion">Add Question</button>
            </div>
        </div>
        </div>
        <ShowComponent :show="postId" :toggle="toggle" @close-popup="hide" :post="post" :comments="com" @newcomment="commentcounter"/>
    </div>
...
<script setup>
    ...
    import ShowComponent from './ShowAdmin.vue';
    ...
    onBeforeMount(() => {
        fetch('api/myquestions')
        .then( res => res.json())
        .then(res => {
            data.myquestions = JSON.parse(JSON.stringify(res.data)),
            newQId.value = data.myquestions[data.myquestions.length-1].id
    })

    })
    // console.log(questions.value)
    const commentcounter = (event) => {
        console.log("Yes!")
    }
    const newQuestion = () => {
        if(!inputQuestion.value == ''){
            data.myquestions.push({
    ...
    const DeleteQuestion = (id, index) => {
        ...
        if (confirm('Are you sure?')) {
            axios.delete('/api/posts/'+ id, {
                ...
                data.myquestions.splice(index, 1)
            ))   
        }
    }

    const EditQuestion = (selected) => {
        editMode.value = !editMode.value
        if(Editid.value == 0){
            Editid.value = selected.id
            OldValue.value = JSON.parse(JSON.stringify(selected))
    ...
    const UpdateQuestion = (question) => {
        if (question.title === '') {
            alert("Error!")
            question.title = OldValue.value.title
        }
        else{
            let id = question.id;
