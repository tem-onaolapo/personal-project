  ...
  <header class="h-16 w-full flex items-center relative px-5 space-x-10 bg-blue-950">
    ...
    <div class="flex justify-end items-center relative w-full">
      ...
      <div class="flex flex-col items-end ">
        ...
        <button class="bg-sky-600 rounded-md text-white ml-2 w-full h-full p-2 font-bold">Post a Question</button>
      </div>
  ...
  <main class="max-w-full h-full flex relative overflow-y-hidden">
    ...
    <div class="h-full w-full m-4 rounded-tl grid-flow-col auto-cols-max gap-4 overflow-y-scroll">
      ...
      <div class="w-full flex flex-wrap gap-4">
        <div class="w-96 h-60 rounded-2xl flex-shrink-0 flex-grow bg-sky-600 hover:bg-sky-500 transition-colors p-8 cursor-pointer"  v-for="post in posts" :key="post.id" @click="show(post.id)">
            <h3 class="text-lg font-bold text-center text-white">{{ post.title }}</h3>
            ...
            <div class="flex mt-8 justify-center">
                ...
                <h3 class=" pl-3 text-sm font-bold text-center text-white align-bottom flex justify-center">{{ post.commentcount }}</h3>
            </div>
